...
export default class Downloadables extends Component {
  constructor(props) {
    super(props)
  ...
  get downloadables () {
    return {
      all: {
        title: 'All Documents',
        files: [this.props.techs && this.props.techs[0].meta.technical_info_pdf]
      },
      tech: {
        title: 'All Tech Data',
        files: this.props.techs && this.props.techs.map(el => el.meta.technical_info_pdf) || []
      },
      bim: {
        title: 'All BIM/Revit',
        files: this.props.bim && this.props.bim.map(el => el.meta.technical_info_pdf) || []
      }
    }
  }

  renderFileList() {
    return this.state.selected.files.map((el, ind) => {
      return <p key={ind}>{el}</p>
    })
  }

  wrapperFunction(func, selected) {
