...
class ProductCheck extends React.Component<Props, State> {
  ...
  render () {
    return <Media query={{ maxWidth: MOBILEMAXWIDTH }}>
      {match =>
        match ? (
          // mobile
          <Collapsible trigger={this.props.title.replace(/_/g, ' ')} triggerTagName={'button'} classParentString={`Collapsible ${style.checkBoxTitle}`} triggerStyle={{ color: '#a7a9ac', fontSize: '13px', fontWeight: '300', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
